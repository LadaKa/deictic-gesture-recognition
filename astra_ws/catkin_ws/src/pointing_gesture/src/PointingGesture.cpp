...
// stashed code:
bool PointingGesture::HasIntersectionInsideFrame(
    int frame_min_x,
    int frame_max_x,
    int frame_min_y,
    int frame_max_y)
{
    bool inside_frame = 
        intersection.x >= frame_min_x
        && intersection.x <= frame_max_x
        && intersection.y >= frame_min_y
        && intersection.y <= frame_max_y;

    if (inside_frame){
        return true;
    }
    
    //printf("Gesture intersection is outside the safety frame, will be ignored.");
    return false;
}
