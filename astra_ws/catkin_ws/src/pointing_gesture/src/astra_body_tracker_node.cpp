...
public:
  ...
  // stashed code:
  // safety from for robot movement
  int safety_frame_min_x = 1;
  int safety_frame_max_x = 3;
  int safety_frame_min_y = 1;
  int safety_frame_max_y = 5;
  ...
  astra_body_tracker_node(std::string name) : _name(name)
  {
    ...
    frame_pub_ = nh_.advertise<visualization_msgs::Marker>("body_tracker/frame", 1);
    ...
    for (int i = 0; i < bodyList.count; ++i)
    {
        ...
        (current_upper_joint_position.z > skeleton_data.joint_position_right_hand.z))
      {
       ...
       // PointingGesture::current_upper_joint = PointingGesture::r_elbow;
        ...
        // stashed code
        if (
          firstPointingGestureDetected
          &&
          !pointingGesture.HasIntersectionInsideFrame(
            safety_frame_min_x,
            safety_frame_max_x,
            safety_frame_min_y,
            safety_frame_max_y))
          {
            // intersection is outside the robot safety frame
            continue;
          }
        ...
        if (secondPointingGestureDetected)
        {
            ...
            for (int i = 0; i<30; i++){
              ros::spinOnce();
              rVizPublisher.PublishTarget(pointingGesture.GetFloorIntersection());
              sleep(1);
            }
...
private:
  ...
  void runAstraStreamLoop()
  {
    ...
    do
    {
      RVizPublisher rVizFramePublisher(frame_pub_);
      rVizFramePublisher.PublishSafetyFrame(
        safety_frame_min_x, safety_frame_max_x, safety_frame_min_y, safety_frame_max_y);
